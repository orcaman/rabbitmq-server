...
-type(amqp_method_field_name() ::
       ( exchange | consumer_tag | reply_text | result
       | details | identifier | nowait | reply_code
       | prefetch_size | reply_text | class_id | delivery_tag
       | nowait | method_id | global | exchange
       | routing_key | mandatory | consume_rate | realm
       | requeue | exclusive | immediate | prefetch_size
       | passive | string_result | exchange | delivery_tag
       | active | consumer_tag | write | ticket
       | content_size | read | operation | routing_key
       | meta_data | integer_4 | ticket | redelivered
       | consumer_tag | integer_1 | prefetch_count | consumer_tag
       | requeue | ticket | routing_key | exchange
       | type | consumer_tag | passive | consumer_tag
       | delivery_tag | durable | auto_delete | routing_key
       | internal | nowait | arguments | exchange
       | consumer_tag | string_2 | routing_key | operation
       | multiple | ticket | exchange | if_unused
       | cluster_id | mandatory | exchange | nowait
       | consumer_tag | message_count | nowait | queue
       | ticket | channel_max | integer_op | ticket
       | delivery_tag | queue | passive | string_op
       | durable | frame_max | ticket | exclusive
       | queue | consumer_tag | auto_delete | global
       | nowait | arguments | requeue | queue
       | exchange | consumer_tag | consumer_tag | message_count
       | consumer_count | no_local | exclusive | content_checksum
       | ticket | delivery_tag | queue | queue
       | routing_key | exchange | mandatory | routing_key
       | nowait | immediate | arguments | identifier
       | ticket | ticket | nowait | arguments
       | reply_code | exchange | delivery_tag | ticket
       | global | queue | integer_2 | nowait
       | multiple | no_ack | version_major | exclusive
       | reply_code | version_minor | message_count | integer_3
       | mechanisms | locales | ticket | server_properties
       | exchange | client_properties | exclusive | if_unused
       | exchange | response | no_ack | nowait
       | prefetch_count | no_local | challenge | delivery_tag
       | staged_size | message_count | response | redelivered
       | delivery_tag | ticket | string_1 | queue
       | heartbeat | reply_code | integer_result | routing_key
       | redelivered | channel_max | frame_max | heartbeat
       | routing_key | reply_text | nowait | queue
       | virtual_host | exchange | consumer_tag | capabilities
       | requeue | dtx_identifier | insist | known_hosts
       | prefetch_count | table | host | queue
       | ticket | known_hosts | routing_key | ticket
       | consumer_tag | reply_code | routing_key | reply_text
       | class_id | reply_text | method_id | identifier
       | mechanism | immediate | no_local | no_ack
       | out_of_band | nowait | if_empty | consumer_tag
       | active | prefetch_size | consumer_tag | reply_text
       | result | queue | active | locale
       | exchange | reply_code )).
