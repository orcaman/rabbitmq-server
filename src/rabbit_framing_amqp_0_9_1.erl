...
-type(amqp_method_field_name() ::
       ( requeue | arguments | immediate | routing_key
       | ticket | delivery_tag | exchange | if_unused
       | no_ack | mandatory | nowait | exclusive
       | routing_key | multiple | ticket | destination
       | source | message_count | nowait | routing_key
       | nowait | arguments | consumer_tag | ticket
       | destination | passive | source | routing_key
       | nowait | consumer_tag | consumer_tag | arguments
       | redelivered | active | delivery_tag | routing_key
       | reply_code | message_count | ticket | queue
       | passive | ticket | durable | server_properties
       | exclusive | auto_delete | nowait | arguments
       | queue | mechanisms | exchange | queue
       | message_count | consumer_count | requeue | exchange
       | ticket | queue | ticket | exchange
       | exchange | routing_key | nowait | arguments
       | if_empty | delivery_tag | multiple | requeue
       | ticket | reply_text | queue | prefetch_count
       | nowait | nowait | consumer_tag | drain
       | requeue | frame_max | heartbeat | available
       ...
       | redelivered | consumer_tag | out_of_band | arguments
       | channel_id | prefetch_size | exchange | passive
       | active | delivery_tag | global | active
       | credit_drained | reply_code | reply_text | ticket
       | class_id | method_id | consumer_tag | consumer_tag
       | no_local | no_ack | realm | queue
       | response | exclusive | queue | ticket
       | version_major | version_minor | routing_key | write
       | read | locales | ticket | exchange
       | client_properties | mechanism | queue | ticket
       | credit | exchange | type | ticket
       | channel_max | challenge | delivery_tag | durable
       | auto_delete | internal | nowait | nowait )).
